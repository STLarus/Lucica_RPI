...
#define MG_UECC_ENABLE_VLI_API 0
	...
	extern "C" {
		...
		/* Sets dest = src. */
		void mg_uecc_vli_set(mg_uecc_word_t *dest,
		...
		void mg_uecc_vli_mult(mg_uecc_word_t *result,
			const mg_uecc_word_t *left,
			const mg_uecc_word_t *right,
			wordcount_t num_words);

		/* Computes result = left^2. Result must be 2 * num_words long. */
		void mg_uecc_vli_square(mg_uecc_word_t *result,
			const mg_uecc_word_t *left,
			wordcount_t num_words);
		...
		/* Computes result = (left + right) % mod.
		   ...
		   mod. */
		void mg_uecc_vli_modAdd(mg_uecc_word_t *result,
			const mg_uecc_word_t *left,
			const mg_uecc_word_t *right,
		...
		/* Computes result = (left - right) % mod.
		   Assumes that left < mod and right < mod, and that result does not overlap
		   mod. */
		void mg_uecc_vli_modSub(mg_uecc_word_t *result,
			const mg_uecc_word_t *left,
			const mg_uecc_word_t *right,
			const mg_uecc_word_t *mod,
			wordcount_t num_words);
		...
		void mg_uecc_vli_mmod(mg_uecc_word_t *result,
		...
		/* Computes result = (left * right) % mod.
		   Currently only designed to work for mod == curve->p or curve_n. */
		void mg_uecc_vli_modMult(mg_uecc_word_t *result,
			const mg_uecc_word_t *left,
			const mg_uecc_word_t *right,
			const mg_uecc_word_t *mod,
			wordcount_t num_words);
		...
		/* Computes result = left^2 % mod.
		   Currently only designed to work for mod == curve->p or curve_n. */
		void mg_uecc_vli_modSquare(mg_uecc_word_t *result,
			const mg_uecc_word_t *left,
			const mg_uecc_word_t *mod,
			wordcount_t num_words);
		...
		void mg_uecc_vli_modInv(mg_uecc_word_t *result,
		...
		void mg_uecc_vli_nativeToBytes(uint8_t *bytes,
			int num_bytes,
			const mg_uecc_word_t *native);
		/* Converts big-endian bytes to an integer in uECC native format. */
		void mg_uecc_vli_bytesToNative(mg_uecc_word_t *native,
			const uint8_t *bytes,
			int num_bytes);
...
#define MG_UECC_WORD_BITS_MASK 0x03F
	...
	struct mg_connection *mg_listen(struct mg_mgr *,
		const char *url,
	...
	struct mg_connection *mg_http_listen(struct mg_mgr *,
		const char *url,
		mg_event_handler_t fn,
		void *fn_data);
	struct mg_connection *mg_http_connect(struct mg_mgr *,
		const char *url,
		mg_event_handler_t fn,
		void *fn_data);
	void mg_http_serve_dir(struct mg_connection *,
	...
	void mg_http_serve_file(struct mg_connection *,
		...
		const char *path,
		const struct mg_http_serve_opts *);
	void mg_http_reply(struct mg_connection *,
		int status_code,
		const char *headers,
		const char *body_fmt,
		...);
...
#define WEBSOCKET_OP_PONG 10
	...
	struct mg_connection *mg_ws_connect(struct mg_mgr *,
		...
		const char *fmt,
		...);
	void mg_ws_upgrade(struct mg_connection *,
		struct mg_http_message *,
		const char *fmt,
		...);
...
#define MQTT_PROP_SHARED_SUBSCRIPTION_AVAILABLE 0x2A
	...
	void mg_mqtt_send_header(struct mg_connection *,
		uint8_t cmd,
		uint8_t flags,
		uint32_t len);
	...
	size_t mg_mqtt_next_prop(struct mg_mqtt_message *,
		struct mg_mqtt_prop *,
		size_t ofs);
...
#define MG_IRAM
	...
	enum {
		...
		MG_OTA_FIRST_BOOT = 1,
		// Device booting the first time after the OTA
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                \
	...
	enum {
		MG_PHY_LEDS_ACTIVE_HIGH = (1 << 0), // Set if PHY LEDs are connected to ground
		  MG_PHY_CLOCKS_MAC = (1 << 1)   // Set when PHY clocks MAC. Otherwise, MAC clocks PHY
	};

	enum { MG_PHY_SPEED_10M, MG_PHY_SPEED_100M, MG_PHY_SPEED_1000M };

	void mg_phy_init(struct mg_phy *, uint8_t addr, uint8_t config);
	bool mg_phy_up(struct mg_phy *,
		uint8_t addr,
		bool *full_duplex,
		uint8_t *speed);
