...
struct mg_flexspi_nor_driver_interface {
	...
	int(*program)(uint32_t instance,
		struct mg_flexspi_nor_config *config,
		uint32_t dst_addr,
		const uint32_t *src);
	uint32_t reserved;
	int(*erase)(uint32_t instance,
		struct mg_flexspi_nor_config *config,
		uint32_t start,
		uint32_t lengthInBytes);
	uint32_t reserved2;
	int(*update_lut)(uint32_t instance,
		uint32_t seqIndex,
...
MG_IRAM static int flexspi_nor_get_config(struct mg_flexspi_nor_config *config) {
	struct mg_flexspi_nor_config default_config = {
		...
		.readSampleClkSrc = 1,
		// ReadSampleClk_LoopbackFromDqsPad
		.csHoldTime = 3,
		.csSetupTime = 3,
...
MG_IRAM bool mg_flash_erase(void *addr) {
	...
	// and 1060
	MG_ARM_DISABLE_IRQ();
	bool ok = (flexspi_nor->erase(FLEXSPI_NOR_INSTANCE,
		&config,
		(uint32_t) dst,
...
MG_IRAM bool mg_flash_write(void *addr, const void *buf, size_t len) {
	...
	while (ok && src < end) {
		...
		if ((char *) buf >= (char *) mg_flash_start()) {
			...
			MG_ARM_DISABLE_IRQ();
			status = flexspi_nor->program(FLEXSPI_NOR_INSTANCE,
				&config,
				(uint32_t) dst_ofs,
				tmp);
		}
...
static void mg_sendnsreq(struct mg_connection *,
	struct mg_str *,
	int,
	struct mg_dns *,
	bool);
...
static size_t mg_dns_parse_name_depth(const uint8_t *s,
	size_t len,
	size_t ofs,
	char *to,
	size_t tolen,
...
static size_t mg_dns_parse_name(const uint8_t *s,
	size_t n,
	size_t ofs,
	char *dst,
	size_t dstlen) {
...
size_t mg_dns_parse_rr(const uint8_t *buf,
	size_t len,
	size_t ofs,
	bool is_question,
	struct mg_dns_rr *rr) {
...
static bool mg_dns_send(struct mg_connection *c,
...
static void mg_sendnsreq(struct mg_connection *c,
	struct mg_str *name,
	int ms,
	struct mg_dns *dnsc,
	bool ipv6) {
...
void mg_call(struct mg_connection *c, int ev, void *ev_data) {
	...
	const char *names[] = {
		"EV_ERROR", "EV_OPEN", "EV_POLL",
		"EV_RESOLVE",
		"EV_CONNECT", "EV_ACCEPT", "EV_TLS_HS", "EV_READ",
		"EV_WRITE", "EV_CLOSE",
		"EV_HTTP_MSG", "EV_HTTP_CHUNK",
...
static size_t scpy(void(*out)(char, void *),
	void *ptr,
	char *buf,
	size_t len) {
	size_t i = 0;
...
size_t mg_vxprintf(void(*out)(char, void *),
	void *param,
	const char *fmt,
	va_list *ap) {
	size_t i = 0, n = 0;
...
bool mg_file_write(struct mg_fs *fs,
	const char *path,
	const void *buf,
	size_t len) {
	bool result = false ;
...
static void ff_list(const char *dir,
...
static void packed_list(const char *dir,
	void(*fn)(const char *, void *),
	void *userdata) {
...
static void p_list(const char *dir,
	void(*fn)(const char *, void *),
	void *userdata) {
...
static void p_list(const char *path,
	void(*fn)(const char *, void *),
	void *userdata) {
	(void) path,
	(void) fn,
...
//
// --xyz--
size_t mg_http_next_multipart(struct mg_str body,
	size_t ofs,
...
void mg_http_bauth(struct mg_connection *c,
	const char *user,
	const char *pass) {
...
int mg_http_get_var(const struct mg_str *buf,
	const char *name,
	char *dst,
	size_t dst_len) {
	int len ;
...
struct mg_str *mg_http_get_header(struct mg_http_message *h, const char *name) {
	...
	struct mg_str *k = &h->headers[i].name,
	*v = &h->headers[i].value ;
...
static bool mg_http_parse_headers(const char *s,
	const char *end,
	struct mg_http_header *h,
	size_t max_hdrs) {
	size_t i,
	n ;
	for(i = 0 ; i < max_hdrs ; i++) {
	struct mg_str k = { NULL, 0 },
	v = { NULL, 0 } ;
	if(s >= end) return false ;
	if(s[0] == '\n' || (s[0] == '\r' && s[1] == '\n')) break ;
	k.buf = (char *) s ;
	while(s < end && s[0] != ':' && (n = clen(s, end)) > 0) s += n,
	k.len += n ;
	...
	h[i].name = k,
	h[i].value = v ;  // Success. Assign values
}
return true ;
}
...
static void mg_http_vprintf_chunk(struct mg_connection *c,
	const char *fmt,
	va_list *ap) {
...
void mg_http_reply(struct mg_connection *c,
	int code,
	const char *headers,
	const char *fmt,
	...) {
...
va_end(ap) ;
if(c->send.len > 16) {
	size_t n = mg_snprintf((char *) &c->send.buf[len - 15],
	11,
	"%-10lu",
...
static struct mg_str s_known_types[] = {
	MG_C_STR("html"),
	MG_C_STR("text/html; charset=utf-8"),
	MG_C_STR("htm"),
	MG_C_STR("text/html; charset=utf-8"),
	MG_C_STR("css"),
	MG_C_STR("text/css; charset=utf-8"),
	MG_C_STR("js"),
	MG_C_STR("text/javascript; charset=utf-8"),
	MG_C_STR("gif"),
	MG_C_STR("image/gif"),
	MG_C_STR("png"),
	MG_C_STR("image/png"),
	...
	MG_C_STR("font/ttf"),
	MG_C_STR("svg"),
	MG_C_STR("image/svg+xml"),
	MG_C_STR("txt"),
	MG_C_STR("text/plain; charset=utf-8"),
	MG_C_STR("avi"),
	MG_C_STR("video/x-msvideo"),
	MG_C_STR("csv"),
	MG_C_STR("text/csv"),
	MG_C_STR("doc"),
	MG_C_STR("application/msword"),
	MG_C_STR("exe"),
	MG_C_STR("application/octet-stream"),
	MG_C_STR("gz"),
	MG_C_STR("application/gzip"),
	MG_C_STR("ico"),
	MG_C_STR("image/x-icon"),
	MG_C_STR("json"),
	MG_C_STR("application/json"),
	MG_C_STR("mov"),
	MG_C_STR("video/quicktime"),
	MG_C_STR("mp3"),
	MG_C_STR("audio/mpeg"),
	MG_C_STR("mp4"),
	MG_C_STR("video/mp4"),
	...
	MG_C_STR("video/mpeg"),
	MG_C_STR("pdf"),
	MG_C_STR("application/pdf"),
	MG_C_STR("shtml"),
	MG_C_STR("text/html; charset=utf-8"),
	MG_C_STR("tgz"),
	MG_C_STR("application/tar-gz"),
	MG_C_STR("wav"),
	MG_C_STR("audio/wav"),
	MG_C_STR("webp"),
	MG_C_STR("image/webp"),
	MG_C_STR("zip"),
	MG_C_STR("application/zip"),
	MG_C_STR("3gp"),
	MG_C_STR("video/3gpp"),
	{ 0, 0 },
};
...
void mg_http_serve_file(struct mg_connection *c,
...
static void listdir(struct mg_connection *c,
	struct mg_http_message *hm,
	const struct mg_http_serve_opts *opts,
	char *dir) {
...
static int uri_to_path2(struct mg_connection *c,
	struct mg_http_message *hm,
	struct mg_fs *fs,
	struct mg_str url,
	struct mg_str dir,
	...
	if (url.len < hm->uri.len) {
		mg_url_decode(hm->uri.buf + url.len,
			hm->uri.len - url.len,
			path + n,
			path_size - n,
			0);
...
static int uri_to_path(struct mg_connection *c,
	struct mg_http_message *hm,
	const struct mg_http_serve_opts *opts,
	char *path,
	size_t path_size) {
...
void mg_http_serve_dir(struct mg_connection *c,
	struct mg_http_message *hm,
	const struct mg_http_serve_opts *opts) {
...
void mg_http_creds(struct mg_http_message *hm,
	char *user,
	size_t userlen,
	char *pass,
	size_t passlen) {
...
o = 0,
	hm.body.len = 0,
	hm.message.len = (size_t) n ;
while((cl = skip_chunk(s + o, len - o, &pl, &dl)) > 0) {
	memmove(s + hm.body.len, s + o + pl, (size_t) dl) ;
	o += cl,
	hm.body.len += (size_t) dl,
...
struct mg_connection *mg_http_connect(struct mg_mgr *mgr,
	const char *url,
...
struct mg_connection *mg_http_listen(struct mg_mgr *mgr,
	const char *url,
	mg_event_handler_t fn,
	void *fn_data) {
...
size_t mg_iobuf_add(struct mg_iobuf *io,
	size_t ofs,
	const void *buf,
	size_t len) {
...
size_t mg_json_next(struct mg_str obj,
  ...
  if(n < 0 || o < 0 || (size_t)(o + n) > sub.len) {
	...
	sub.buf += o + n,
	sub.len -= (size_t)(o + n) ;
	while(sub.len > 0 && *sub.buf != ':') sub.len--,
	sub.buf++ ;
	if(sub.len > 0 && *sub.buf == ':') sub.len--,
	sub.buf++ ;
	n = 0,
	o = mg_json_get(sub, "$", &n) ;
	if(n < 0 || o < 0 || (size_t)(o + n) > sub.len) {
...
int mg_json_get(struct mg_str json, const char *path, int *toklen) {
	...
	int ci = -1,
	ei = -1 ;  // Current and expected index in array
...
bool mg_json_get_bool(struct mg_str json, const char *path, bool *v) {
	int found = 0,
	off = mg_json_get(json, path, NULL) ;
...
bool mg_json_unescape(struct mg_str s, char *to, size_t n) {
	...
	if(mg_str_to_num(mg_str_n(s.buf + i + 2, 4),
	16,
	&to[j],
...
char *mg_json_get_b64(struct mg_str json, const char *path, int *slen) {
	...
	size_t k = mg_base64_decode(json.buf + off + 1,
	(size_t)(len - 2),
	result,
	(size_t) len) ;
...
char *mg_json_get_hex(struct mg_str json, const char *path, int *slen) {
	...
	int i ;
	for(i = 0 ; i < len - 2 ; i += 2) {
	mg_str_to_num(mg_str_n(json.buf + off + 1 + i, 2),
	16,
	&result[i >> 1],
	sizeof(uint8_t)) ;
}
...
void mg_mqtt_send_header(struct mg_connection *c,
	uint8_t cmd,
	uint8_t flags,
	uint32_t len) {
...
int mg_mqtt_parse(const uint8_t *buf,
	size_t len,
	uint8_t version,
...
struct mg_connection *mg_mqtt_connect(struct mg_mgr *mgr,
	const char *url,
	const struct mg_mqtt_opts *opts,
	mg_event_handler_t fn,
	void *fn_data) {
...
struct mg_connection *mg_mqtt_listen(struct mg_mgr *mgr,
	const char *url,
	mg_event_handler_t fn,
	void *fn_data) {
	struct mg_connection *c = mg_listen(mgr, url, fn, fn_data) ;
	if(c != NULL) c->pfn = mqtt_cb,
	c->pfn_data = mgr ;
	return c ;
}
...
static bool mg_aton6(struct mg_str str, struct mg_addr *addr) {
	...
	dc = 42 ;
	addr->scope_id = 0 ;
	if(str.len > 2 && str.buf[0] == '[') str.buf++,
	str.len -= 2 ;
...
struct mg_connection *mg_alloc_conn(struct mg_mgr *mgr) {
	struct mg_connection *c = (struct mg_connection *) calloc(1, sizeof(*c) + mgr->extraconnsize) ;
	  if(c != NULL) {
	c->mgr = mgr ;
...
struct mg_connection *mg_listen(struct mg_mgr *mgr,
	const char *url,
	mg_event_handler_t fn,
	void *fn_data) {
	struct mg_connection *c = NULL ;
...
struct mg_connection *mg_wrapfd(struct mg_mgr *mgr,
	int fd,
	mg_event_handler_t fn,
	void *fn_data) {
...
#if MG_ENABLE_EPOLL
	      if(mgr->epoll_fd >= 0) close(mgr->epoll_fd),
	mgr->epoll_fd = -1 ;
#endif
	        mg_tls_ctx_free(mgr) ;
}
...
static struct ip *tx_ip(struct mg_tcpip_if *ifp,
	uint8_t *mac_dst,
	uint8_t proto,
	uint32_t ip_src,
	uint32_t ip_dst,
...
static void tx_udp(struct mg_tcpip_if *ifp,
	uint8_t *mac_dst,
	uint32_t ip_src,
	uint16_t sport,
	uint32_t ip_dst,
...
static void tx_dhcp(struct mg_tcpip_if *ifp,
	uint8_t *mac_dst,
	uint32_t ip_src,
	uint32_t ip_dst,
	uint8_t *opts,
...
static void tx_dhcp_request_sel(struct mg_tcpip_if *ifp,
	...
	uint8_t opts[] = {
		...
		54,
		4,
		0, 0, 0, 0, // DHCP server ID
		50,
		4,
		0, 0, 0, 0, // Requested IP
		255                       // End of options
	};
...
static void tx_dhcp_request_re(struct mg_tcpip_if *ifp,
	uint8_t *mac_dst,
	uint32_t ip_src,
	uint32_t ip_dst) {
	uint8_t opts[] = {
...
static struct mg_connection *getpeer(struct mg_mgr *mgr,
	struct pkt *pkt,
	bool lsn) {
	struct mg_connection *c = NULL;
...
static size_t tx_tcp(struct mg_tcpip_if *ifp,
	uint8_t *dst_mac,
	uint32_t dst_ip,
	uint8_t flags,
	uint16_t sport,
...
static size_t tx_tcp_pkt(struct mg_tcpip_if *ifp,
	struct pkt *pkt,
	uint8_t flags,
	uint32_t seq,
	const void *buf,
	...
	return tx_tcp(ifp,
		...
		pkt->tcp->sport,
		seq,
		mg_htonl(mg_ntohl(pkt->tcp->seq) + delta),
		buf,
		len);
}
...
static void read_conn(struct mg_connection *c, struct pkt *pkt) {
	...
	if (pkt->tcp->flags & TH_FIN) {
		...
		tx_tcp((struct mg_tcpip_if *) c->mgr->priv,
			s->mac,
			rem_ip,
			flags,
			c->loc.port,
...
bool mg_ota_rollback(void) {
	...
	if (mg_flash_bank() == 0) {
		...
		return mg_flash_save(NULL,
			MG_OTADATA_KEY + MG_FIRMWARE_PREVIOUS,
			&prev,
			sizeof(prev));
	}
	else {
...
MG_IRAM void mg_ota_boot(void) {
	...
	if (curr.status == MG_OTA_FIRST_BOOT) {
		...
		mg_flash_save(NULL,
			MG_OTADATA_KEY + MG_FIRMWARE_CURRENT,
			&prev,
			sizeof(prev));
		mg_flash_save(NULL,
			MG_OTADATA_KEY + MG_FIRMWARE_PREVIOUS,
...
static size_t qcpy(void(*out)(char, void *),
	void *ptr,
	char *buf,
	size_t len) {
	size_t i = 0, extra = 0;
...
static size_t bcpy(void(*out)(char, void *),
	void *arg,
	uint8_t *buf,
	size_t len) {
	size_t i, j, n = 0;
...
void mg_sha1_update(mg_sha1_ctx *context,
...
void mg_sha256_update(mg_sha256_ctx *ctx,
	const unsigned char *data,
	size_t len) {
	size_t i;
	for (i = 0; i < len; i++) {
...
void mg_hmac_sha256(uint8_t dst[32],
	uint8_t *key,
	size_t keysz,
	uint8_t *data,
	size_t datasz) {
...
static MG_SOCKET_TYPE raccept(MG_SOCKET_TYPE sock,
	union usa *usa,
	socklen_t *len) {
	MG_SOCKET_TYPE fd = MG_INVALID_SOCKET;
	do {
...
bool mg_wakeup(struct mg_mgr *mgr,
...
void mg_http_serve_ssi(struct mg_connection *c,
	const char *root,
	const char *fullpath) {
...
i++,
	...
	caps--,
	caps->len = 0 ;  // Restart previous cap
}
} else {
	return false ;
}
...
bool mg_span(struct mg_str s, struct mg_str *a, struct mg_str *b, char sep) {
	...
	if(b && len < s.len) b->buf++,
	b->len-- ;        // Skip separator
	return true ;
}
}
...
int mg_aes_gcm_decrypt(unsigned char *output,
	const unsigned char *input,
	size_t input_length,
	const unsigned char *key,
	const size_t key_len,
...
enum mg_tls_hs_state {
	...
	MG_TLS_STATE_CLIENT_WAIT_FINISHED,
	// Wait for Finished
...
#if 1
static void mg_ssl_key_log(const char *label,
...
static int mg_der_find(uint8_t *der,
	size_t dersz,
	uint8_t *oid,
	size_t oidsz,
...
static void mg_tls_derive_secret(const char *label,
	uint8_t *key,
	size_t keysz,
	uint8_t *data,
	size_t datasz,
...
static void mg_tls_generate_handshake_keys(struct mg_connection *c) {
	...
	mg_tls_derive_secret("tls13 derived",
		early_secret,
		32,
		zeros_sha256_digest,
		32,
		pre_extract_secret,
		32);
	mg_hmac_sha256(tls->handshake_secret,
		pre_extract_secret,
		sizeof(pre_extract_secret),
		tls->x25519_sec,
	...
	mg_tls_derive_secret("tls13 s hs traffic",
		...
		server_hs_secret,
		32);
	mg_tls_derive_secret("tls13 key",
		server_hs_secret,
		32,
		NULL,
		0,
	...
	mg_tls_derive_secret("tls13 iv",
		...
		tls->server_write_iv,
		12);
	mg_tls_derive_secret("tls13 finished",
		server_hs_secret,
		32,
		NULL,
		0,
		tls->server_finished_key,
		32);
	...
	mg_ssl_key_log("SERVER_HANDSHAKE_TRAFFIC_SECRET",
		tls->random,
		server_hs_secret,
		32);
	mg_ssl_key_log("CLIENT_HANDSHAKE_TRAFFIC_SECRET",
		tls->random,
		client_hs_secret,
		32);
}
...
static void mg_tls_generate_application_keys(struct mg_connection *c) {
	...
	mg_tls_derive_secret("tls13 s ap traffic",
		master_secret,
		32,
		hash,
		32,
	...
	mg_tls_derive_secret("tls13 iv",
		...
		tls->server_write_iv,
		12);
	mg_tls_derive_secret("tls13 c ap traffic",
		master_secret,
		32,
		hash,
		32,
...
static void mg_tls_encrypt(struct mg_connection *c,
	const uint8_t *msg,
	size_t msgsz,
	uint8_t msgtype) {
	struct tls_data *tls = (struct tls_data *) c->tls;
	...
	if (c->is_client) {
		mg_aes_gcm_encrypt(outmsg,
			outmsg,
			msgsz + 1,
			tls->client_write_key,
			sizeof(tls->client_write_key),
			nonce,
			sizeof(nonce),
			associated_data,
...
static void mg_tls_server_send_cert_verify(struct mg_connection *c) {
	...
	mg_uecc_sign_deterministic(tls->server_key,
		hash,
		sizeof(hash),
		&ctx.uECC,
		sig,
...
static int mg_tls_client_recv_ext(struct mg_connection *c) {
	...
	if (tls->recv.buf[0] != MG_TLS_ENCRYPTED_EXTENSIONS) {
		mg_error(c,
			"expected server extensions but got msg 0x%02x",
			tls->recv.buf[0]);
		return -1;
	}
	mg_tls_drop_message(c);
...
static int mg_tls_client_recv_cert(struct mg_connection *c) {
	...
	if (tls->recv.buf[0] != MG_TLS_CERTIFICATE) {
		mg_error(c,
			"expected server certificate but got msg 0x%02x",
			tls->recv.buf[0]);
		return -1;
	}
	...
	do {
		...
		if (mg_der_find(cert,
			certsz,
			(uint8_t *) "\x2A\x86\x48\xCE\x3D\x03\x01\x07",
			8,
			&oid) < 0) {
	...
	do {
		...
		while (seq.len > 0) {
			...
			if (mg_match(mg_str((const char *) tls->hostname),
				mg_str_n((const char *) subj.value, subj.len),
				NULL)) {
				subj_match = 1;
				break;
			}
...
static int mg_tls_client_recv_cert_verify(struct mg_connection *c) {
	...
	if (tls->recv.buf[0] != MG_TLS_CERTIFICATE_VERIFY) {
		mg_error(c,
			"expected server certificate verify but got msg 0x%02x",
			tls->recv.buf[0]);
		return -1;
	}
...
static int mg_tls_client_recv_finish(struct mg_connection *c) {
	...
	if (tls->recv.buf[0] != MG_TLS_FINISHED) {
		mg_error(c,
			"expected server finished but got msg 0x%02x",
			tls->recv.buf[0]);
		return -1;
	}
	mg_tls_drop_message(c);
...
static int mg_parse_pem(const struct mg_str pem,
	const struct mg_str label,
	struct mg_str *der) {
	size_t n = 0, m = 0;
	char *s;
	const char *c;
...
void mg_tls_ctx_init(struct mg_mgr *mgr) {
	...
	if (ctx == NULL) {
		...
		if ((rc = mbedtls_ssl_ticket_setup(&ctx->tickets,
			mg_mbed_rng,
			NULL,
...
void mg_tls_init(struct mg_connection *c, const struct mg_tls_opts *opts) {
	...
	SSL_set_session_id_context(tls->ssl,
		(const uint8_t *) id,
		(unsigned) strlen(id));
...
struct MG_UECC_Curve_t {
	...
	void(*double_jacobian)(mg_uecc_word_t *X1,
		mg_uecc_word_t *Y1,
		mg_uecc_word_t *Z1,
		MG_UECC_Curve curve);
...
#endif
	void(*x_side)(mg_uecc_word_t *result,
		const mg_uecc_word_t *x,
		MG_UECC_Curve curve);
...
static void muladd(mg_uecc_word_t a,
	mg_uecc_word_t b,
	mg_uecc_word_t *r0,
	mg_uecc_word_t *r1,
	mg_uecc_word_t *r2) {
...
#if !asm_square
static void mul2add(mg_uecc_word_t a,
	mg_uecc_word_t b,
	mg_uecc_word_t *r0,
	mg_uecc_word_t *r1,
	mg_uecc_word_t *r2) {
...
static void vli_modInv_update(mg_uecc_word_t *uv,
	const mg_uecc_word_t *mod,
	wordcount_t num_words) {
	mg_uecc_word_t carry = 0;
	if (!EVEN(uv)) {
...
static void double_jacobian_default(mg_uecc_word_t *X1,
	...
	mg_uecc_vli_modAdd(Z1,
		X1,
		X1,
		curve->p,
		num_words); /* t3 = 2*(x1^2 - z1^4) */
	mg_uecc_vli_modAdd(X1,
		X1,
		Z1,
		curve->p,
...
static void x_side_default(mg_uecc_word_t *result,
	const mg_uecc_word_t *x,
	MG_UECC_Curve curve) {
...
static void vli_mmod_fast_secp160r1(mg_uecc_word_t *result,
	...
	while (mg_uecc_vli_cmp_unsafe(result,
		curve_secp160r1.p,
...
static void vli_mmod_fast_secp192r1(uint8_t *result, uint8_t *product) {
	...
	while (carry || mg_uecc_vli_cmp_unsafe(curve_secp192r1.p,
		result,
...
static void mod_sqrt_secp224r1_rp(mg_uecc_word_t *d1,
	...
	/* f0 <-- p  - c */
	mg_uecc_vli_modSub(f0,
		curve_secp224r1.p,
		c,
		curve_secp224r1.p,
		num_words_secp224r1);
	for (i = 0; i <= 6; i++) {
		mod_sqrt_secp224r1_rss(d1,
			e1,
			f1,
			d0,
			e0,
...
static void vli_mmod_fast_secp224r1(uint8_t *result, uint8_t *product) {
	...
	if (carry < 0) {
		...
		while (carry || mg_uecc_vli_cmp_unsafe(curve_secp224r1.p,
			result,
			num_words_secp224r1) != 1) {
			carry -= mg_uecc_vli_sub(result,
				result,
				curve_secp224r1.p,
				num_words_secp224r1);
		}
...
static void vli_mmod_fast_secp224r1(uint64_t *result, uint64_t *product) {
	...
	if (carry < 0) {
		...
		while (mg_uecc_vli_cmp_unsafe(curve_secp224r1.p,
			result,
			num_words_secp224r1) != 1) {
...
static void vli_mmod_fast_secp256r1(uint8_t *result, uint8_t *product) {
	...
	if (carry < 0) {
		...
		while (carry || mg_uecc_vli_cmp_unsafe(curve_secp256r1.p,
			result,
...
static void vli_mmod_fast_secp256r1(uint32_t *result, uint32_t *product) {
	...
	if (carry < 0) {
		do {
			carry += (int) mg_uecc_vli_add(result,
				result,
				curve_secp256r1.p,
				num_words_secp256r1);
		} while (carry < 0);
		...
		while (carry || mg_uecc_vli_cmp_unsafe(curve_secp256r1.p,
			result,
			num_words_secp256r1) != 1) {
			carry -= (int) mg_uecc_vli_sub(result,
				result,
				curve_secp256r1.p,
				num_words_secp256r1);
		}
...
static void double_jacobian_secp256k1(mg_uecc_word_t *X1,
	...
	mg_uecc_word_t *Z1,
	MG_UECC_Curve curve);
static void x_side_secp256k1(mg_uecc_word_t *result,
	const mg_uecc_word_t *x,
	MG_UECC_Curve curve);
#if (MG_UECC_OPTIMIZATION_LEVEL > 0)
...
static void apply_z(mg_uecc_word_t *X1,
	mg_uecc_word_t *Y1,
	const mg_uecc_word_t *const Z,
	MG_UECC_Curve curve) {
...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P', Q => P + Q
*/
static void XYcZ_add(mg_uecc_word_t *X1,
	mg_uecc_word_t *Y1,
	mg_uecc_word_t *X2,
	mg_uecc_word_t *Y2,
	MG_UECC_Curve curve) {
...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P - Q, Q => P + Q
*/
static void XYcZ_addC(mg_uecc_word_t *X1,
	mg_uecc_word_t *Y1,
	mg_uecc_word_t *X2,
	mg_uecc_word_t *Y2,
	MG_UECC_Curve curve) {
...
static void EccPoint_mult(mg_uecc_word_t *result,
	const mg_uecc_word_t *point,
	const mg_uecc_word_t *scalar,
	const mg_uecc_word_t *initial_Z,
	bitcount_t num_bits,
	MG_UECC_Curve curve) {
	/* R0 and R1 */
...
static mg_uecc_word_t EccPoint_compute_public_key(mg_uecc_word_t *result,
	...
	EccPoint_mult(result,
		curve->G,
		p2[!carry],
		initial_Z,
		(bitcount_t)(curve->num_n_bits + 1),
		curve);
...
int mg_uecc_make_key(uint8_t *public_key,
	uint8_t *private_key,
	MG_UECC_Curve curve) {
#if MG_UECC_VLI_NATIVE_LITTLE_ENDIAN
	...
	for (tries = 0; tries < MG_UECC_RNG_MAX_TRIES; ++tries) {
		if (!mg_uecc_generate_random_int(_private,
			curve->n,
...
int mg_uecc_shared_secret(const uint8_t *public_key,
...
#else
	mg_uecc_vli_bytesToNative(_private,
		private_key,
		BITS_TO_BYTES(curve->num_n_bits));
...
#if MG_UECC_SUPPORT_COMPRESSED_POINT
void mg_uecc_compress(const uint8_t *public_key,
	uint8_t *compressed,
	MG_UECC_Curve curve) {
	wordcount_t i;
...
void mg_uecc_decompress(const uint8_t *compressed,
	uint8_t *public_key,
	MG_UECC_Curve curve) {
#if MG_UECC_VLI_NATIVE_LITTLE_ENDIAN
...
static void bits2int(mg_uecc_word_t *native,
	const uint8_t *bits,
	unsigned bits_size,
	MG_UECC_Curve curve) {
...
#if MG_UECC_VLI_NATIVE_LITTLE_ENDIAN
	bcopy((uint8_t *) signature + curve->num_bytes,
		(uint8_t *) s,
		curve->num_bytes);
#else
...
int mg_uecc_sign_with_k(const uint8_t *private_key,
	const uint8_t *message_hash,
	unsigned hash_size,
	const uint8_t *k,
	uint8_t *signature,
	...
	return mg_uecc_sign_with_k_internal(private_key,
		message_hash,
		hash_size,
		k2,
		signature,
...
int mg_uecc_sign(const uint8_t *private_key,
	const uint8_t *message_hash,
	unsigned hash_size,
	uint8_t *signature,
	MG_UECC_Curve curve) {
	...
	for (tries = 0; tries < MG_UECC_RNG_MAX_TRIES; ++tries) {
		if (!mg_uecc_generate_random_int(k,
			curve->n,
			BITS_TO_WORDS(curve->num_n_bits))) {
			return 0;
		}

		if (mg_uecc_sign_with_k_internal(private_key,
			message_hash,
			hash_size,
			k,
			signature,
...
int mg_uecc_verify(const uint8_t *public_key,
	const uint8_t *message_hash,
	unsigned hash_size,
...
void mg_uecc_point_mult(mg_uecc_word_t *result,
	const mg_uecc_word_t *point,
	const mg_uecc_word_t *scalar,
	MG_UECC_Curve curve) {
	mg_uecc_word_t tmp1[MG_UECC_MAX_WORDS];
...
static void condswap(limb_t a[2 * NLIMBS],
	limb_t b[2 * NLIMBS],
	limb_t doswap) {
	unsigned i;
...
static void x25519_core(mg_fe xs[5],
...
int mg_tls_x25519(uint8_t out[X25519_BYTES],
	const uint8_t scalar[X25519_BYTES],
...
size_t mg_ws_vprintf(struct mg_connection *c,
	int op,
	const char *fmt,
	va_list *ap) {
	size_t len = c->send.len;
...
static void ws_handshake(struct mg_connection *c,
	const struct mg_str *wskey,
	const struct mg_str *wsproto,
	const char *fmt,
	va_list *ap) {
...
size_t mg_ws_send(struct mg_connection *c,
	const void *buf,
	size_t len,
	int op) {
	uint8_t header[14];
...
struct mg_connection *mg_ws_connect(struct mg_mgr *mgr,
	const char *url,
	mg_event_handler_t fn,
	void *fn_data,
	const char *fmt,
...
void mg_ws_upgrade(struct mg_connection *c,
	struct mg_http_message *hm,
	const char *fmt,
	...) {
...
// Transmit frame
static size_t mg_tcpip_driver_imxrt_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
...
enum { // ID1  ID2
  ...
  MG_PHY_DP83867 = 0xa231,
	// 2000 a231 - TI DP83867I
	...
	if (config & MG_PHY_CLOCKS_MAC) {
	  ...
	  // Enable 50 MHz external ref clock at XI (preserve defaults)
		if (id1 == MG_PHY_DP83x && id2 != MG_PHY_DP83867) {
			...
			phy->write_reg(phy_addr,
				MG_PHY_KSZ8x_REG_PC2R,
...
// Transmit frame
static size_t mg_tcpip_driver_ra_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
	static int s_txno; // Current descriptor index
	if (len > sizeof(s_txbuf[ETH_DESC_CNT])) {
...
static size_t mg_tcpip_driver_same54_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
...
static size_t mg_tcpip_driver_stm32f_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
	if (len > sizeof(s_txbuf[s_txno])) {
		MG_ERROR(("Frame too big, %ld", (long) len));
...
static size_t mg_tcpip_driver_stm32h_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
	if (len > sizeof(s_txbuf[s_txno])) {
		MG_ERROR(("Frame too big, %ld", (long) len));
...
static uint32_t s_txno;
static size_t mg_tcpip_driver_tm4c_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
	if (len > sizeof(s_txbuf[s_txno])) {
		MG_ERROR(("Frame too big, %ld", (long) len));
...
static void w5500_txn(struct mg_tcpip_spi *s,
	uint8_t block,
	uint16_t addr,
	bool wr,
	void *buf,
...
static size_t w5500_tx(const void *buf,
	size_t buflen,
	struct mg_tcpip_if *ifp) {
	struct mg_tcpip_spi *s = (struct mg_tcpip_spi *) ifp->driver_data;
...
static size_t mg_tcpip_driver_xmc_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
	if (len > sizeof(s_txbuf[s_txno])) {
		MG_ERROR(("Frame too big, %ld", (long) len));
...
static size_t mg_tcpip_driver_xmc7_tx(const void *buf,
	size_t len,
	struct mg_tcpip_if *ifp) {
	if (len > sizeof(s_txbuf[s_txno])) {
		MG_ERROR(("Frame too big, %ld", (long) len));
